...
6. **Physics and light are written into the prompt, then checked in the output.** Relight to the plate, name the light direction, ground the feet, move the hair with the wind, keep the light constant. QC rejects anything that fails, however pretty. One scene, one camera: the character and the background share the same sharpness, grain, colour and white balance (phone deep focus), shadows fall the same way, and hair and clothing obey gravity; never ask for a softer or blurred background. Same phone, same way: if the character films herself, every clip and still is the same front-camera selfie framing from the same phone; never mix friend-held, propped or gimbal shots, which read as a pasted layer with a different zoom. Keep a wardrobe library in the bible and rotate outfits and headwear so no outerwear repeats within 3 posts.
...
1. **Prototype intake (if the user has images):** pick the sharpest, most neutral, front-facing image as the anchor. Name the drift between images honestly (different face shape, hair texture, brow colour) and say which features you are locking. Derive 2 to 3 anchors only from what the face-anchor photo actually shows (brow colour vs hair, eye shape, a real mole), never invent marks it does not have: an invented feature (e.g. freckles) gets painted on by every model. The face-anchor photo is the only identity source for every agent. Earrings and makeup are per-scene styling that should vary by day, not identity anchors.
